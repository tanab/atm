...
bool MySqlCache::initCache() {
    ...
    call_once(once, [this] {
        ...
        statement.reset(m_connection->prepareStatement(
            "SELECT category_id1, category_id2, type, sources, resulting_category, inflection FROM "
            "compatibility_rules ORDER BY type, category_id1, categor_id2 ASC"));
        resultSet.reset(statement->executeQuery());
...
const std::shared_ptr<PrefixRows> MySqlCache::prefixTable() const { return m_prefix_rows; }
